...
# 변수 셋팅
s3_bucket=$(grep "s3bucket" /sorc001/BATCH/COM/ENV/anenv.ini | sed 's/ = /|/g' | cut -d '|' -f2)
...
while true; do
    ...
    #파일 갯수 체크
    filecnt=$(find /data001/sapdata/$s3_key* -type f | wc -l)
    ...
    if [ -f "$end_file_path" ]; then
        ...
        if [ $filecnt -eq 2 ]; then
            echo "파일이 1건 존재합니다."
            aws s3 cp $file_path/$s3_key.CSV s3://$s3_bucket/$s3_key.CSV
        elif [ $filecnt -gt 2 ]; then
            csvfilecnt=$(expr $filecnt - 1)
            echo "파일이 $csvfilecnt건 존재합니다."
            for (( counter = 1; counter < $filecnt; counter++)); do
                cnt=$(printf "%02d\n" $counter)
                aws s3 cp $file_path/$s3_key"_"$cnt.CSV s3://$s3_bucket/$s3_key"_"$cnt.CSV
            done
        else
            echo "파일을 확인하세요."
            exit 1
