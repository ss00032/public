...
# 테이블 정의서 레이아웃 출력 (운영계)
#SELECT 'S' || A.TABLE_NAME
#    , 'S' || A.TABLE_COMMENT
#    , B.COLUMN_NAME
#    , B.COLUMN_COMMENT
#    , B.ORDINAL_POSITION
#    , CASE WHEN B.DATA_TYPE = 'datetime'
#           THEN 'TIMESTAMP'
#           WHEN B.DATA_TYPE = 'int'
#           THEN 'INTEGER'
#           ELSE UPPER(B.DATA_TYPE)
#       END AS DATA_TYPE
#    , CASE WHEN B.DATA_TYPE = 'varchar' OR B.DATA_TYPE = 'char'
#           THEN B.CHARACTER_MAXIMUM_LENGTH
#           WHEN B.DATA_TYPE = 'decimal'
#           THEN B.NUMERIC_PRECISION || ',' || B.NUMERIC_SCALE
#           ELSE ''
#       END AS LENGTH
#    , CASE WHEN B.IS_NULLABLE = 'NO'
#           THEN 'NOT NULL'
#           ELSE ''
#       END AS NULLABLE
...
#WHERE A.TABLE_SCHEMA = 'MDSDEV'
#  AND A.TABLE_NAME IN ('PR_PDCT_CRFN_INFO', 'MK_ITGR_EVN_D', 'MK_MD_EVN_D', 'MK_MD_PIPD_EVN_D', 'MK_DPSTR_BULD_M', 'MK_EVN_ADVS_CAEV_CMMN_D', 'MK_MRKT_EVN_D', 'MK_PRF_EVN_CND_D', 'MK_DCNT_EVN_CND_D', 'PF_SALS_BYDY_BYMD_BYSTMN')
...
try {
     ...
     for ($i=2; $i -le $rowcount; $i++) {
         ...
         $stgcomtable = $tableID.Substring(4, $tableID.Length-4)
         $stgtable = $tableID1.Substring(4, $tableID1.Length-4)
         ...
         if ($tableID -eq $tableID1) {
             ...
             if ($j -eq 1) {
                 ...
                 $createDDL += "--------------------------------------------------------------------------------"
                 $createDDL += "DROP TABLE IF EXISTS ansor."  + $tableID1 + ";"
                 $createDDL += "CREATE TABLE ansor." + $tableID1 + " ("
                 $createDDL += $columnArr
                 $createDDL += ");"
                 $createDDL += ""
                 $createDDL += "COMMENT ON TABLE ansor." + $tableID1 + " IS '" + $tableName1 + "';"
                 $createDDL += $columnNameArr
                 $createDDL += ""
                 #$createDDL += "grant select on ansor." + $tableID1 + " to group select_dept_dw_group ;"
                 #$createDDL += "grant all on ansor." + $tableID1 + " to group all_dept_dw_group ;"
                 ...
                 if ($tableName1.Contains("스냅샷")) {
                     ...
                     $createstgDDL += "--------------------------------------------------------------------------------"
                     $createstgDDL += "DROP TABLE IF EXISTS anstg."  + $stgtable + ";"
                     $createstgDDL += "CREATE TABLE anstg." + $stgtable + " ("
                     $createstgDDL += $columnstgArr
                     $createstgDDL += ");"
                     $createstgDDL += ""
                     $createstgDDL += "COMMENT ON TABLE anstg." + $stgtable + " IS '" + $stgtableName + "';"
                     $createstgDDL += $columnNamestgArr
                     $createstgDDL += ""
                     #$createstgDDL += "grant select on anstg." + $stgtable + " to group select_dept_dw_group ;"
                     #$createstgDDL += "grant all on anstg." + $stgtable + " to group all_dept_dw_group ;"
         ...
         if ($columnID -eq "ETL_WRK_DTM") {
             ...
             $columnstgArr += "  " + $columnID.PadRight(30) + " " + $dataType.PadRight(15) + " " + $nullAble
         }
         #    if ($default -is [double]) {
         ...
         $columnNameArr += "COMMENT ON COLUMN ansor." + $tableID + "." + $columnID + " IS '" + $columnName + "';"
         if ($columnID -eq "ETL_WRK_DTM") {

         } else {
             $columnNamestgArr += "COMMENT ON COLUMN anstg." + $stgcomtable + "." + $columnID + " IS '" + $columnName + "';"
         }
     ...
     $createDDLAll > C:\Users\Owner\Documents\source\DDL\tableALL.sql
     $createstgDDLAll > C:\Users\Owner\Documents\source\DDL\stgtableALL.sql
