...
`;"
             ...
             while ($result.Read())
             {
              ...
              if ($Excelchgkey -eq "UPD_DTM") {
                  ...
                  } elseif ($tableid -eq "SIV_TDMR_BYDY_MD_S") {
              ...
              if ($Excelchgyn -eq 'S') {
                  ...
                  if ($Excelchgyn -eq 'Y') {
                      if ($tableid -eq "SIV_TDMR_BYDY_MD_S" -or $tableid -eq "SIV_TDMR_BYMN_MD_S" -or $tableid -eq "SPF_SALS_ACR_YMD" -or $tableid -eq "SPF_ORD_ACR_YMD") {
                          $deletecolarr = "DELETE /* " + ($ExcelTable + "(" + $pgmLCDid + ") */`n  FROM " + $ExcelOwner + "." + $ExcelTable).PadRight(149) + "/* " + $ExcelTableNm + " */`n WHERE " + $LCDchgKey + $betweenarr
                      } else {
                          $deletecolarr = "DELETE /* " + ($ExcelTable + "(" + $pgmLCDid + ") */`n  FROM " + $ExcelOwner + "." + $ExcelTable).PadRight(149) + "/* " + $ExcelTableNm + " */`n WHERE EXISTS (`n               SELECT 1`n                 FROM " + $TempOwner + "." + $TempTable.PadRight(72) + "/* " + $ExcelTableNm + " */`n" + $deletepk + "              )"
                      }
