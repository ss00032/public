...
`;"
             ...
             while ($result.Read())
             {
              ...
              if ($tableid -eq "SSA_SALS_TRAN_PCAL_P" -or $tableid -eq "SMK_TRAN_DCNT_EVN_HDER_P" -or $tableid -eq "SIV_PIPD_RCDS_BYDY_S" -or $tableid -eq "SSA_SALS_VIP_MLG_P" -or $tableid -eq "SIV_TDMR_BYDY_MD_S" -or 
                  $tableid -eq "SSA_SALS_CASH_P" -or $tableid -eq "SSS_PIPD_BYMR_SALS_S" -or $tableid -eq "SIV_PIPD_RCDS_BYMN_S" -or $tableid -eq "SSS_BYMD_BYMR_SALS_S" -or $tableid -eq "SSS_BYCAHR_BYMD_BYMR_SALS_S" -or
                  $tableid -eq "SSA_SALS_HDER_P" -or $tableid -eq "SMK_TRAN_DCNT_APY_P" -or $tableid -eq "SSA_SALS_PDCT_P" -or $tableid -eq "SSA_SALS_GFCET_P" -or $tableid -eq "SIV_GRPST_DTL" -or 
                  $tableid -eq "SPO_ORDE_DTL" -or $tableid -eq "SPO_PCH_DTL" -or $tableid -eq "SMK_TRAN_DCNT_EVN_APY_P" -or $tableid -eq "SIV_TDMR_BYMN_MD_S" -or $tableid -eq "SSA_SALS_SSG_PNT_SLMG_P" -or $tableid -eq "SSA_SALS_CAD_P" -or 
                  $tableid -eq "SPF_SALS_BYDY_BYMD_BYSTMN" -or $tableid -eq "SSA_SALS_SSG_PNT_P" -or $tableid -eq "SPR_NPPD_INFO_SHOP_PRC" -or 
                  $tableid -eq "SIV_ANYT_INVT_SUV" -or $tableid -eq "SMK_TRAN_DCNT_EVN_DTL_ITEM_P"  -or $tableid -eq "SSA_ORD_ONOFL_ORD_P"
                  # -or $tableid -eq "SPR_SHOP_PIPD_PRC"  -or $tableid -eq "SMK_MRKT_EVN_D"  -or $tableid -eq "SMK_MRKT_EVN_STR_MD_D"
              ) {
                  $impleChk = "Y"
                  for ($a=4; $a -le $implrowcount; $a++) {
                      $implTableId = $implExcelWorkSheet.cells.Item($a, 4).value2
                      $implChgKey = $implExcelWorkSheet.cells.Item($a, 11).value2
                      if($tableid -eq $implTableId) {
                          $chgKey = $implChgKey
                          continue
                      }
                  }
              ...
              if ($Excelchgyn -eq 'S') {
                  ...
                  if ($Excelchgyn -eq 'Y') {
                      $deletecolarr = "DELETE /* " + ($ExcelTable + "(" + $pgmLCDid + ") */`n  FROM " + $ExcelOwner + "." + $ExcelTable).PadRight(90) + "/* " + $ExcelTableNm + " */`n WHERE EXISTS (`n               SELECT 1`n                 FROM " + $TempOwner + "." + $TempTable.PadRight(72) + "/* " + $ExcelTableNm + " */`n" + $deletepk + "              )"
                      $truncatearr = "TRUNCATE /* " + ($ExcelTable + "(" + $pgmLCDid + ") */" + $ExcelOwner + "." + $ExcelTable).PadRight(88) + "/* " + $ExcelTableNm + " */"
                      $truncatetmparr = "TRUNCATE /* " + ($ExcelTable + "(LCD_" + $ExcelTable + "_TG) */ " + $TempOwner + "." + $TempTable).PadRight(88) + "/* " + $ExcelTableNm + " */"
                      $impldeletecolarr = "DELETE /* " + ($ExcelTable + "(" + $pgmLIDid + ") */`n  FROM " + $ExcelOwner + "." + $ExcelTable).PadRight(90) + "/* " + $ExcelTableNm + " */`n WHERE " + $chgKey + $implbetweenarr
                  } else {
                  ...
                  if ($Excelchgyn -eq 'Y') {
                      $tempinsertarr = "INSERT /* " + $ExcelTable + "(" + $pgmLCDid + ") */`n  INTO " + $TempOwner + "." + $TempTable.PadRight(87) + "/* " + $ExcelTableNm.Substring(1, $ExcelTableNm.Length-1) + " */`n     (`n" + $tempselcolarr + "     )`nSELECT /* " + $ExcelTable + "(" + $pgmLCDid + ") */`n" + $tempinscolarr + "  FROM `"bludbev`".`"" + $Excelasisowner + "`".`"" + ($ExternalTable + "`"").PadRight(72) + "/* " + $Excelasistbnm + " */`n WHERE `"" + $Excelchgkey + "`"" + $betweenarr
...
`####SQL
"
[System.IO.File]::WriteAllLines("C:\Users\Owner\Documents\source\body\이행대상\LID_$ExcelTable`_TG.sql",$bodyimplesql,$utf8NoBomEncoding)
         }
         if ($Exceljuje -eq "SAP") {
...
`IAM_ROLE '{ARN_S3}'
`DELIMITER '|'
`ACCEPTINVCHARS ESCAPE BLANKSASNULL EMPTYASNULL ENCODING UTF8
`####SQL
`/**************************************************************************************************/
`/*  P_BAT_STG_NM : Target Delete                                                                  */
`/**************************************************************************************************/
