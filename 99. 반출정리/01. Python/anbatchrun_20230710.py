...
try:
    ...
    pgmnmquery = f"select COALESCE(obj_description('ansor.{table_name}'::regclass), '없음')"
        ...
        for sql in sql_array:
            ...
            if (sql.strip() != ""):
                try:
                    ...
                    formatted_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S") + "{:06d}".format(datetime.datetime.now().microsecond)
                    formatted_time = formatted_time[:17]
