...
from sgd import config_3                                          # copy option 테스트
...
class L0LoadOperator(BaseOperator):
    ...
    def __init__(self,
        ...
        if len(self.copy_options) == 0:
            self.copy_options = config_3.redshift_env['copy_options'][self.company_code]
        self.autocommit = autocommit
        self.working_schema = f"{config_3.sgd_env['wk_layer']}_{company_code}"
        self.s3_stg_bucket = f"{config_3.s3_env['stg_bucket_prefix']}-{self.company_code}-{self.use_purpose}-1"
    ...
    def execute(self, context):
        ...
        redshift_conn_id = config_3.conn_id['dl_redshift'][self.use_purpose]
    ...
    def _copy_to_redshift_from_s3(self, conn, schema, table):
        ...
        copy_query = f"""
            ...
            iam_role '{config_3.redshift_env['iam_role'][self.use_purpose]}'
            csv quote as '\036'
            delimiter '|' ;
        """
