...
class L0LoadOperator(BaseOperator):
    ...
    """
    template_fields = ('execution_kst','s3_key','delete_sql_for_merge','select_sql_for_insert','insert_query',)
    template_ext = ()
    ui_color = '#99e699'
    ...
    def __init__(self,
        ...
        if len(self.copy_options) == 0:
            self.copy_options = config_2.redshift_env['copy_options'][self.company_code]
        self.autocommit = autocommit
        self.working_schema = f"{config_2.sgd_env['wk_layer']}_{company_code}"
        self.s3_stg_bucket = f"{config_2.s3_env['stg_bucket_prefix']}-{self.company_code}-{self.use_purpose}-1"
    ...
    def _copy_to_redshift_from_s3(self, conn, schema, table):
        ...
        copy_query = f"""
            ...
            iam_role '{config_2.redshift_env['iam_role'][self.use_purpose]}'
            {copy_options};
        """
