...
class StgToBckOperator(BaseOperator):
    ...
    def __init__(self,
        ...
        self.s3_key=s3_key
        self.source_bucket_name = f"{config_2.s3_env['stg_bucket_prefix']}-{self.company_code}-{self.use_purpose}-1"
        self.dest_bucket_name = f"{config_2.s3_env['bck_bucket_prefix']}-{self.company_code}-{self.use_purpose}-1"
        self.aws_conn_id = config_2.conn_id['dl_s3']
        self.verify = verify
