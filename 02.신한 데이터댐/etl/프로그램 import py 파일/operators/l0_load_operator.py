...
class L0LoadOperator(BaseOperator):
    """
    ...
    """
    template_fields = ('s3_key', 'target_schema', 'target_table', 'copy_options')
    template_ext = ()
    ui_color = '#99e699'
    ...
    def __init__(self,
        ...
        self.s3_bck_bucket = f"{config.s3_env['bck_bucket_prefix']}-{self.company_code}-{self.use_purpose}-1"
    ...
    def execute(self, context):
        ...
        self.log.info(f"""
            ...
            s3_key={self.s3_key}, table_load_type={self.table_load_type}, 
            company_code={self.company_code}, use_purpose={self.use_purpose},
            delete_sql_for_merge={self.delete_sql_for_merge}
            """)
        ...
        # TODO: if select_sql_for_unload is not empty: do_unload()
        # TODO: data file backup
    ...
    def _copy_to_redshift_from_s3(self, conn, schema, table):
        ...
        self.log.info(f"Copy done: {copy_query}")
    ...
    def _delete_tgt_tbl_for_merge(self, conn):
        ...
        self.log.info(f"Target table deleted for merge: {affected_rows} deleted. \n{self.delete_sql_for_merge}")
    ...
    def _check_validation(self):
        if not (utils.valid_table_load_type(self.table_load_type)
                or utils.valid_company_code(self.company_code)
